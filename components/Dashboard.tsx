...
const NeonTuner: React.FC = () => {
  ...
  const barColor = cents === null ? "bg-orange-500" : Math.abs(cents)<=5 ? "bg-green-400" : "bg-red-500";
  const barWidth = Math.max(-50, Math.min(50, cents || 0));

  return (
    <div className="w-full">
      <button onClick={()=>setIsOpen(!isOpen)} className="w-full py-2 px-4 bg-green-500 text-black rounded mb-2">
      ...
      {isOpen && (
        <div className="flex flex-col items-center w-full p-2 bg-black/40 rounded-lg border border-green-500/50">
          ...
          <div className="relative w-full h-4 bg-black/20 rounded overflow-hidden mt-1">
            <div className={`absolute h-full ${barColor} transition-all`}
              style={{ width: `${Math.abs(barWidth)}%`, left: barWidth>0?"50%":`${50+barWidth}%` }}
            />
          </div>
...
const TaskItem: React.FC<{ task: ProgressItem }> = ({ task }) => {
  const statusColors = { Completed:"text-green-500","In Progress":"text-yellow-400","Not Started":"text-red-500" };
  ...
  return (
    <li className="flex flex-col p-3 bg-black/70 rounded-md border border-green-500/20">
      <div className="flex justify-between items-start">
        <p className="font-bold text-green-500 flex-1">{task.category}: <span className="font-normal text-white">{task.detail}</span></p>
        <span className={`font-mono text-sm mt-1 ${statusColors[status]}`}>{status}</span>
      </div>
      {task.resource_links.length>0 && <div className="mt-2 space-y-1">{task.resource_links.map((l,i)=><ResourceLink key={i} url={l.url}/>)}</div>}
    </li>
  );
};
  ...
  return (
    <div className="border border-green-500/40 rounded-lg">
      <button className="w-full flex justify-between p-4 bg-black/60 hover:bg-black/70 transition" onClick={()=>setIsOpen(!isOpen)}>
        <div><h3 className="text-xl text-green-500">{grade}</h3>{isCurrent && <p className="text-xs text-green-400/70">Current</p>}</div>
        <ChevronDownIcon className={`w-6 h-6 text-green-500 transition-transform ${isOpen?"rotate-180":""}`} />
      </button>
      <div className="p-4"><ProgressBar label={`${completed}/${total} completed`} percentage={percent}/></div>
      {isOpen && <ul className="p-4 pt-0 space-y-2">{tasks.length===0?<p className="text-center text-white/60">No tasks for this grade.</p>:tasks.map((t,i)=><TaskItem key={i} task={t}/>)}</ul>}
    </div>
  );
};
...
const Dashboard: React.FC<{student:Student & {previous_grades:string},progressData:ProgressItem[],onLogout:()=>void}> = ({student,progressData,onLogout})=>{
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen py-8 px-4 flex justify-center items-start" style={{backgroundImage:'url("https://raw.githubusercontent.com/MJTGuitar/site-assets/06a843085b182ea664ac4547aca8948d0f4e4886/Guitar%20Background.png")',backgroundSize:"cover",backgroundPosition:"center"}}>
        <div className="w-full max-w-4xl bg-black/80 p-6 border border-green-500/50 rounded-lg backdrop-blur">
          ...
          <div className="flex items-center justify-center mb-2 space-x-1">
            <img src="/images/logo.png" alt="MJT Guitar Tuition" className="w-24 h-24 object-contain neon-glow-pulse opacity-70"/>
          ...
          <div className="space-y-6">
            {gradeList.map(g=><GradeSection key={g} grade={g} tasks={gradesMap[g]||[]} isCurrent={g===normalizeGrade(student.current_grade)}/>)}
          </div>
        </div>
      </div>
    </ErrorBoundary>
