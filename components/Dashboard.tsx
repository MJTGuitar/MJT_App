    ...
    <ErrorBoundary>
      ...
      >
        <div className="w-full max-w-4xl bg-black/80 p-6 border border-green-500/50 rounded-lg backdrop-blur">
          {/* Header with logo */}
          <header className="flex flex-col items-start pb-4 border-b border-green-500/70 mb-6">
            <img
              src="/images/logo.png"
              alt="MJT Guitar Tuition"
              className="w-40 h-40 object-contain opacity-70 mb-2"
            />
            <h1 className="text-3xl text-white font-bold">Student Dashboard</h1>
            <p className="text-white">Welcome, {student.student_name}!</p>
            <button
              onClick={onLogout}
              className="mt-2 px-4 py-2 border border-red-500/70 text-red-400 rounded hover:bg-red-500/10"
            >
          ...
          <div className="mb-8">
            ...
            >
              {tunerOpen ? "Hide Tuner" : "Show Tuner"}
            </button>
            {tunerOpen && (
              <div className="mt-2 bg-black/40 p-4 border border-green-500/50 rounded-lg flex flex-col items-center">
                <NeonTuner />
              </div>
            )}
          </div>
